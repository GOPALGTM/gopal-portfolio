  ...
  return (
    <section id="experience" className="max-w-5xl mx-auto px-4 sm:px-6 py-16">
      ...
      <div className="relative">
        ...
        <div className="absolute left-5 top-0 bottom-0 w-1 bg-cyan/30 rounded" />
        <ul className="space-y-16 pl-12">
          {experiences.map((exp, idx) => (
            <li key={idx} className="relative">
              {/* Icon Circle */}
              <div className="absolute -left-7 top-3 bg-cyan shadow-neon rounded-full p-3">
                <FaBriefcase className="text-black w-5 h-5" />
              </div>
              {/* Experience Card */}
              <div className="bg-white/5 border border-cyan/50 rounded-3xl p-6 shadow-neon hover:shadow-xl transition-shadow cursor-default">
                <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-3 gap-1">
                  <h3 className="text-xl font-bold text-white">{exp.role}</h3>
                  <span className="text-cyan font-mono">{exp.period}</span>
                </div>
                <div className="text-white/70 font-mono mb-4">{exp.company} &bull; {exp.location}</div>
